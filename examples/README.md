...
| [java](./java) | Java / Spring Boot 2.7.x | `io.github.insol-2021:cas-client:2.0.0` (java-cas-client) | 9105 | One-time: `mvn install -DskipTests` in `packages/java-cas-client`, then in `examples/java`: `set -a; . ./.env; set +a; mvn spring-boot:run` → http://localhost:9105 | Local Maven install: the package is published to `~/.m2` via `mvn install`, then declared as a normal `<dependency>` on `io.github.insol-2021:cas-client:2.0.0` in `pom.xml`. |
